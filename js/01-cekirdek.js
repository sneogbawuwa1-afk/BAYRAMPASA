...
// Bu uygulamaya ait IndexedDB veritabanının tamamını temizler. Erişim anahtarı
// localStorage'da tutulduğu için, bulut sıfırlamasından sonra bağlantının yeniden
// kurulabilmesi amacıyla bu işlem anahtara dokunmaz.
async function idbTemizle(){
  const db = await idbAc();
  return new Promise((resolve, reject)=>{
    const tx = db.transaction(IDB_STORE, 'readwrite');
