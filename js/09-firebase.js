...
const CANLI_DINLENEN_ANAHTARLAR = [
  ...
  'efaturaPanelZincirVknListesi_v1',
  'efaturaPanelFaturaSubeAtamalari_v1',
  'efaturaPanelTutarTolerans_v1',
];

// RTDB kuralları doğrudan remove() işlemini kabul etmez; her kaydın erişim
// anahtarını taşımasını zorunlu kılar. Bu nedenle kayıtları silmek yerine, tek
// atomik güncellemede uygulamanın boş varsayılanlarıyla değiştiriyoruz.
async function buluttakiVerileriSifirla(){
  const firebaseVarMi = await firebaseBaslat();
  const erisimAnahtari = erisimAnahtariniAl();
  if(!firebaseVarMi || !erisimAnahtari){
    throw new Error('Bulut bağlantısı veya erişim anahtarı hazır değil.');
  }
  const cihazId = await cihazIdAl();
  const guncellemeZamani = new Date().toISOString();
  const bosDegerler = {
    'efaturaPanelKaynaklar_v1': {earsiv:null, efaturaQnb:null, efaturaLogo:null, musteriKesan:null, musteriBayrampasa:null, efesEkstre:null, netsis:null},
    'efaturaPanelManuel_v1': {},
    'efaturaPanelSonRapor_v1': null,
    'efaturaPanelDonemler_v1': {},
    'efaturaPanelSubeAtamalari_v1': {},
    'efaturaPanelZincirVknListesi_v1': [],
    'efaturaPanelFaturaSubeAtamalari_v1': {},
    'efaturaPanelTutarTolerans_v1': 1,
  };
  const guncellemeler = {};
  Object.entries(bosDegerler).forEach(([anahtar, deger])=>{
    guncellemeler[rtdbYoluGuvenliHalGetir(anahtar)] = {
      deger,
      guncellemeZamani,
      cihazId,
      erisimAnahtari,
  ...
  await _rtdb.ref(KV_YOLU).update(guncellemeler);
  buluttaBaglantiDurumunuGuncelle(true);
}
...
function canliGuncellemeUygula(anahtar, yeniDeger){
  ...
  switch(anahtar){
    ...
    case 'efaturaPanelSubeAtamalari_v1':
      ...
      break;
    case 'efaturaPanelZincirVknListesi_v1':
      state.zincirVknListesi = new Set(Array.isArray(yeniDeger) ? yeniDeger : []);
      if(state.rapor){
        state.rapor = computeRapor(state.kaynaklar, state.manuel, state.subeAtamalari, state.zincirVknListesi, state.faturaSubeAtamalari);
        yenidenCizVeBildir('Zincir VKN listesi başka bir cihazda güncellendi.');
      }
      break;
    case 'efaturaPanelFaturaSubeAtamalari_v1':
      state.faturaSubeAtamalari = new Map(Object.entries(yeniDeger || {}));
      if(state.rapor){
        state.rapor = computeRapor(state.kaynaklar, state.manuel, state.subeAtamalari, state.zincirVknListesi, state.faturaSubeAtamalari);
        yenidenCizVeBildir('Fatura şube atamaları başka bir cihazda güncellendi.');
      }
      break;
    case 'efaturaPanelTutarTolerans_v1':
      if(typeof yeniDeger === 'number' && !isNaN(yeniDeger)){
        tutarToleransiAyarla(yeniDeger);
