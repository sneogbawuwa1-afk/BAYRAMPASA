...
function veriSifirlamaModaliniAc(){
  if(document.getElementById('veriSifirlamaOverlay')) return;
  const overlay = document.createElement('div');
  overlay.className = 'upload-overlay';
  overlay.id = 'veriSifirlamaOverlay';
  overlay.innerHTML = `
    <div class="upload-modal" style="max-width:440px;">
      <div class="upload-modal-head"><div class="upload-modal-title">Verileri Sıfırla</div></div>
      <div style="color:rgba(255,255,255,.72);font-size:13px;line-height:1.6;margin-bottom:18px;">
        Bu işlem yüklenen dosyaları, raporları, dönem arşivini, manuel notları ve şube atamalarını geri alınamaz biçimde temizler.
      </div>
      <div style="display:flex;flex-direction:column;gap:9px;">
        <button type="button" class="sb-olustur" id="btnYerelSifirla" style="margin:0;background:rgba(255,255,255,.12);">Sadece bu cihazdaki verileri sıfırla</button>
        <button type="button" class="sb-olustur" id="btnTumVeriyiSifirla" style="margin:0;background:linear-gradient(135deg,#C84040,#E24B4A);">Yerel ve bulut verilerini sıfırla</button>
        <button type="button" class="sb-olustur" id="btnVeriSifirlamaIptal" style="margin:0;background:transparent;border:1px solid rgba(255,255,255,.16);color:rgba(255,255,255,.72);box-shadow:none;">Vazgeç</button>
      </div>
    </div>`;
  document.body.appendChild(overlay);
  const kapat = ()=> overlay.remove();
  overlay.querySelector('#btnVeriSifirlamaIptal').addEventListener('click', kapat);
  overlay.addEventListener('click', (e)=>{ if(e.target === overlay) kapat(); });
  overlay.querySelector('#btnYerelSifirla').addEventListener('click', async ()=>{
    if(!confirm('Bu cihazdaki tüm uygulama verileri silinecek. Devam etmek istiyor musunuz?')) return;
    await verileriSifirla(false);
    kapat();
  });
  overlay.querySelector('#btnTumVeriyiSifirla').addEventListener('click', async ()=>{
    if(!confirm('Yerel ve buluttaki tüm uygulama verileri geri alınamaz biçimde sıfırlanacak. Devam etmek istiyor musunuz?')) return;
    const butonlar = overlay.querySelectorAll('button');
    butonlar.forEach(b=>{ b.disabled = true; });
    try{
      await verileriSifirla(true);
      kapat();
    }catch(e){
      butonlar.forEach(b=>{ b.disabled = false; });
      alert('Bulut verileri sıfırlanamadı. Yerel veriler korunuyor.\n\n' + (e.message || e));
    }
  });
}

async function verileriSifirla(bulutDahil){
  // Önce bulutu temizle: bağlantı/yetki sorunu olursa yereldeki çalışan veri korunur.
  if(bulutDahil) await buluttakiVerileriSifirla();
  await idbTemizle();
  Object.keys(state.kaynaklar).forEach(k=>{ state.kaynaklar[k] = null; });
  state.rapor = null;
  state.manuel = {};
  state.donemler = {};
  state.goruntulenenDonemId = null;
  state.subeAtamalari = new Map();
  state.zincirVknListesi = new Set();
  state.faturaSubeAtamalari = new Map();
  tutarToleransiAyarla(1);
  const toleransInput = document.getElementById('toleransInput');
  if(toleransInput) toleransInput.value = TUTAR_TOLERANS;
  const topbarSub = document.getElementById('topbarSub');
  if(topbarSub) topbarSub.textContent = 'Henüz veri yüklenmedi';
  aktifGrup='tumu'; aktifDurum='tumu'; aktifKaynak='tumu'; aramaMetni='';
  sayfayiSifirla();
  renderUploadPanels();
  guncelleRaporOlusturButonu();
  renderKPIs();
  ...
  renderGroupSections();
  renderDonemPaneli();
  if(typeof toastGoster === 'function') toastGoster(bulutDahil ? 'Yerel ve bulut verileri sıfırlandı' : 'Bu cihazdaki veriler sıfırlandı', 'basarili');
}
...
async function initApp(){
  ...
  document.getElementById('btnVeriSifirla').addEventListener('click', veriSifirlamaModaliniAc);
